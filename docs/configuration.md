...
Konfiguracja dwupoziomowa: globalny `config.ini` w katalogu repo + opcjonalny `config.ini` w katalogu projektu (ma pierwszeństwo). Przez Web UI: zakładka **Settings** → **Re-analyze with these settings** zapisuje zmiany do `config.ini` projektu automatycznie.

### `[scene_detection]`

| Klucz | Domyślnie | Opis |
|-------|-----------|------|
| `threshold` | `20` | Czułość detektora cięć (PySceneDetect). Niższy = więcej cięć. Zakres typowy: 10–40. |
...
Two-level configuration: global `config.ini` in the repo root + optional per-project `config.ini` (takes precedence). Via Web UI: **Settings** tab → **Re-analyze with these settings** writes changes to the project's `config.ini` automatically.

### `[scene_detection]`

| Key | Default | Description |
|-----|---------|-------------|
| `threshold` | `20` | Scene cut detector sensitivity. Lower = more cuts. Range typically 10–40. |
...
Prompts can be auto-generated by Claude — describe the ride in the **New project** form or **Settings** tab, click **Generate CLIP prompts**.
...
| `max_per_file_sec` | `45` | Max total seconds from one source file. Scenes exceeding this cap are shown in Gallery with a "limit" badge. |
